...
class GoResNet(GoBaseNet):

    def __init__(self, shape, n_class):
        super(GoResNet, self).__init__(shape, n_class)

    def build(self, block_num=2, filters=32, factor=4, se=True,
              drop_out_rate=0.5, activation='swish',regul_l2=0.0001):

        inputs = Input(shape=self.shape)
        x = self._conv_block(inputs, filters, (1, 1), activation='relu')

        for i in range(block_num):
            x = self.get_iteration_block(x, filters, (3, 3), activation='relu')
