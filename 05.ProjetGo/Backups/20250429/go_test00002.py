...
def test_batch32(config):
    config['experiment_name'] = 'Test00002-MobileNet-Batch32'
...
def test_batch64(config):
    config['experiment_name'] = 'Test00002-MobileNet-Batch64'
    config['train']['batch'] = 64
    model = GoMobileNet((19, 19, 31), 361)
    model = model.build(block_num=config['model']['block_num'] , filters=config['model']['filters'] , factor=config['model']['factor'],
                        se=config['model']['se'], drop_out_rate=config['model']['drop_out_rate'], activation=config['model']['activation'])
    model.summary()
...
def test_batch128(config):
    config['experiment_name'] = 'Test00002-MobileNet-Batch128'
    config['train']['batch'] = 128
    model = GoMobileNet((19, 19, 31), 361)
    model = model.build(block_num=config['model']['block_num'] , filters=config['model']['filters'] , factor=config['model']['factor'],
                        se=config['model']['se'], drop_out_rate=config['model']['drop_out_rate'], activation=config['model']['activation'])
    model.summary()
...
if __name__ == "__main__":
    ...
    title, model, val, all_history, val_loss_history, total_time, lrs = test_batch32(config=CONFIG)
    ...
    print(f'-------------------- Model GoMobileNet Batch64-------------------- ')
    title, model, val, all_history, val_loss_history, total_time, lrs = test_batch64(config=CONFIG)
    plot_learning_rate(lrs)
    ...
    print(f'-------------------- Model GoMobileNet Batch128-------------------- ')
    title, model, val, all_history, val_loss_history, total_time, lrs = test_batch128(config=CONFIG)
    plot_learning_rate(lrs)
