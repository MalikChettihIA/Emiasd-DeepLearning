...
def test_gomobilenet(nb_cycle, epochs=EPOCHS):
    ...
    model = model.build(block_num=3, filters=64, factor=4, se=True, drop_out_rate=0.2, activation='swish')
    model.summary()
    ...
    return f'Model GoMixNet - Nb CosineDecay Cycle is {nb_cycle}', model, val, all_history, val_loss_history, total_time, lrs
