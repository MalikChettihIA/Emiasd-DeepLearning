...
def test_gomobilenet(nb_cycle, epochs=EPOCHS):
    ...
    model = model.build(block_num=40, filters=128, factor=4, se=True, drop_out_rate=0.2, activation='swish')
    model.summary()
    ...
    return (f'Model GoMobileNet block_num=40, filters=128, factor=4'), model, val, all_history, val_loss_history, total_time, lrs
...
if __name__ == "__main__":
    ...
    titles = []
    for nb_cycle in [1]:
        print(f'-------------------- Model GoMobileNet - block_num=40, filters=128, factor=4 -------------------- ')
