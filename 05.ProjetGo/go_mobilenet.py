...
class GoMobileNet(GoBaseNet):
    ...
    def build(self, block_num=2, filters=32, factor=4, se=True,
              drop_out_rate=0.5, activation='swish', kernel_size=(3, 3), regul_l2=0.0001):

        inputs = Input(shape=self.shape)
        x = self._conv_block(inputs, filters, (1, 1), activation='relu')

        for i in range(block_num):
            x = self._nested_bottleneck_block(x, filters, kernel_size, factor, se, activation=activation)
        ...
        value_head = Dense(50, kernel_regularizer=regularizers.l2(regul_l2),
